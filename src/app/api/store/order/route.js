...
export async function POST(req) {
    ...
    try {
        ...
        if (!validatedData.success) {
            return new NextResponse(JSON.stringify({ errors: validatedData.error.errors }), { status: 400 });
        }
        ...
        if (!storeInfo?.bank?.subaccountCode) {
            return new NextResponse("This store cannot accept payments yet! try contacting them for more infomation", {
                status: 400,
            });
        }
        ...
        const products = await db.query.product.findMany({
            where: and(or(inArray(product.id, productIds), inArray(product.slug, productIds)), eq(product.storeId, orderInfo.storeId)),
        ...
        await db.transaction(async (tx) => {
            ...
            if (!createdOrder[0].id)
                throw Error("Failed to create order");
            await tx.insert(orderItem).values(orderInfo.items.map((item) => {
                // console.log(item);
