...
export function CreateFeatureLimitDialog({ children, onSuccess, }) {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="planType" render={({ field }) => (<FormItem>
              ...
              <FormField control={form.control} name="featureKey" render={({ field }) => (<FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {featureOptions.map((option) => (<SelectItem key={option.value} value={option.value}>
                            {option.label}
                          </SelectItem>))}
                      </SelectContent>
                    </Select>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="limitType" render={({ field }) => (<FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {limitTypeOptions.map((option) => (<SelectItem key={option.value} value={option.value}>
                            <div>
              ...
              <FormField control={form.control} name="limitValue" render={({ field }) => (<FormItem>
                    <FormLabel>Limit Value</FormLabel>
                    <FormControl>
                      <Input type="number" placeholder="-1 for unlimited, 0 to disable" {...field}/>
                    </FormControl>
                    <FormDescription>
            ...
            <FormField control={form.control} name="resetPeriod" render={({ field }) => (<FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      {resetPeriodOptions.map((option) => (<SelectItem key={option.value} value={option.value}>
                          {option.label}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
            ...
            <FormField control={form.control} name="enabled" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
            ...
            <FormField control={form.control} name="description" render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Optional description for this feature limit" {...field}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
