...
import db from "@/db";
import { userPlans, featureLimits, featureUsage, featureOverrides, user, store, product, } from "@/db/schema";
export class FeatureLimitService {
        ...
        try {
            ...
            if (override) {
                ...
                return {
                    ...
                    current,
                    message: current + requestedAmount > override.overrideValue
            ...
            if (!userPlan) {
                // No plan = free plan
                return this.checkFeatureLimit("free", featureKey, userId, requestedAmount);
            }
            return this.checkFeatureLimit(userPlan.planType, featureKey, userId, requestedAmount);
        }
        catch (error) {
     ...
     */
    static async getCurrentUsage(userId, featureKey) {
        try {
    ...
    static async trackUsage(userId, featureKey, amount = 1, metadata) {
        try {
            ...
            const featureLimit = await db
                .select()
                .from(featureLimits)
                .where(and(eq(featureLimits.planType, planType), eq(featureLimits.featureKey, featureKey), eq(featureLimits.enabled, true)))
                .limit(1);
     ...
     */
    static async getAllFeatureLimits() {
        return db
            .select()
            .from(featureLimits)
     ...
     */
    static async upsertFeatureLimit(planType, featureKey, limitType, limitValue, resetPeriod = "never", enabled = true, description) {
        try {
            const existing = await db
                .select()
                .from(featureLimits)
                .where(and(eq(featureLimits.planType, planType), eq(featureLimits.featureKey, featureKey)))
                .limit(1);
            const now = new Date();
            if (existing.length) {
                // Update existing
     ...
     */
    static async deleteFeatureLimit(planType, featureKey) {
        await db
     ...
     */
    static async createFeatureOverride(userId, featureKey, overrideValue, reason, expiresAt, createdBy) {
        try {
            // Remove existing override first
            await db
                .delete(featureOverrides)
                .where(and(eq(featureOverrides.userId, userId), eq(featureOverrides.featureKey, featureKey)));
            // Create new override
     ...
     */
    static async removeFeatureOverride(userId, featureKey) {
        await db
            .delete(featureOverrides)
            .where(and(eq(featureOverrides.userId, userId), eq(featureOverrides.featureKey, featureKey)));
    }
     ...
     * Assign plan to user
     */
    static async assignUserPlan(userId, planType, endDate, stripeSubscriptionId, stripePriceId) {
        try {
    ...
    static async seedDefaultLimits() {
        ...
        for (const limit of defaultLimits) {
            try {
                await this.upsertFeatureLimit(limit.planType, limit.featureKey, limit.limitType, limit.limitValue, limit.resetPeriod, true, limit.description);
    ...
    // =============================================
    static async checkFeatureLimit(planType, featureKey, userId, requestedAmount) {
        ...
        return {
            ...
            message: !allowed
                ? `${featureKey.replace("_", " ")} limit reached (${currentUsage}/${limit.limitValue} used). ${limit.resetPeriod === "monthly"
                    ? "Resets monthly."
    ...
    static async getActiveOverride(userId, featureKey) {
    ...
    static getNextResetDate(resetPeriod) {
        const now = new Date();
        switch (resetPeriod) {
            case "monthly":
    ...
    static getSuggestedUpgrade(currentPlan) {
        switch (currentPlan) {
            case "free":
                return "pro";
            case "pro":
