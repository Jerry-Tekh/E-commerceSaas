"use server";
import { customerSignUpSchema, } from "@/schemas/customer";
import db from "@/db";
...
export const signOutCustomer = async (storeSlug) => {
    const token = (await cookies()).get("customer_token")?.value;
    if (token) {
        try {
            const decoded = verify(token, process.env.JWT_SECRET);
...
export const signInCustomer = async ({ storeId, ...values }) => {
    try {
        const existingCustomer = await db.query.customer.findFirst({
            where: and(eq(customer.email, values.email), eq(customer.storeId, storeId)),
        });
        if (!existingCustomer) {
            return {
        ...
        const passwordMatch = await bcrypt.compare(values.password, existingCustomer.password);
        if (!passwordMatch) {
            return {
                error: "Invalid email or password",
                success: "",
        ...
        const session = await db
            ...
            .returning();
        const token = sign({ sessionId: session[0].id }, process.env.JWT_SECRET);
        await db
...
export const signUpCustomer = async ({ storeId, ...values }) => {
    try {
...
export const revokeCustomerSession = async (sessionId) => {
    try {
        const session = await db.query.customerSesssion.findFirst({
